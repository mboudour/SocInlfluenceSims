...
def observe():
    ...
    nx.draw_networkx_nodes(g,pos=g.pos,cmap = cm.hsv, vmin = -1, vmax = 1,nodelist=[i for i in g if i!=g.nodes()[x]],
    node_size=2000,node_color=[g.node[i]['state'] for i in g if i !=g.nodes()[x]])#,alpha=.5)

    nx.draw_networkx_nodes(g,cmap = cm.hsv, vmin = -1, vmax = 1,pos=g.pos,nodelist=[g.nodes()[x]],
        node_shape='s',node_size=2000,node_color='r')
